...
single_register_read_write is targeted to help customers create a new CustomLogic example. Developers can update the design, verification, and build flow to meet their needs without having to tear down a separate example. We recommend going through other CL examples before creating a new CL.
...
single_register_read_write contains the minimum required set of design files. All examples require a SystemVerilog file (.sv) named after the CL example and a Verilog header file (.vh) with required defines.
...
[single_register_read_write.sv](./design/single_register_read_write.sv) with the following:
...
[single_register_read_write_defines.vh](./design/single_register_read_write_defines.vh) with the following defines:
...
- Basic test tasks: [$AWS_FPGA_REPO_DIR/hdk/common/verif/include/common_base_test.svh](./../../../common/verif/include/common_base_test.svh)
  - The [base test](./verif/tests/single_register_read_write_base_test.sv) uses `report_pass_fail_status()`
...
single_register_read_write does not yet support stand alone software tests. Please refer to other examples such as [CL_DRAM_HBM_DMA](./../cl_dram_hbm_dma#software)
...
2. Update [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl) with relevant IP's, constraints, etc.
...
See the following `tcl` scripts for more details on the build flow (they are sourced in [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl)):
...
- [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl) - Refer to [STEP 5: Build and Deploy the Design](#step-5-build-and-deploy-the-design) below for more information.
...
- [cl_synth_user.xdc](./build/constraints/cl_synth_user.xdc) - Custom constraints applied to the synthesizer (called in [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl))
- [cl_timing_user.xdc](./build/constraints/cl_timing_user.xdc) - Custom timing constraints applied to the synthesizer (called in [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl))
...
## single_register_read_write Quick Start Guide
...
The first step is to copy this example into a new directory and replace all references to single_register_read_write with the new example name. To automatically create your own example, run [create_new_cl.py](./../create_new_cl.py) from the `$AWS_FPGA_REPO_DIR/hdk/cl/examples` directory
...
single_register_read_write=single_register_read_write
echo "Creating $single_register_read_write directory"
cp -r $single_register_read_write $NEW_CL_NAME

echo "Replacing single_register_read_write with $single_register_read_write in all files"
grep -rl $single_register_read_write $NEW_CL_NAME | xargs sed -i "s/$single_register_read_write/$NEW_CL_NAME/g"

echo "Updating single_register_read_write with $single_register_read_write in all file names"
single_register_read_write=$single_register_read_write NEW_CL_NAME=$NEW_CL_NAME find $NEW_CL_NAME -name "*$single_register_read_write*" -exec sh -c 'mv "$0" "${$0/$single_register_read_write/$NEW_CL_NAME}"' {} \;
```

The result will be a directory containing everything in single_register_read_write (renamed to the new CL name) with functional tools and scripts. Please note that these files need to be manually updated to include any design specific changes.

### STEP 2: Add Design Files

All CL design files should be placed under the [design](./design) directory. Please see the [Design](#design) section for information on the single_register_read_write design files.
...
Once design files have been added, run the [single_register_read_write_base_test](./verif/tests/single_register_read_write_base_test.sv). This test only powers up the test bench to make sure the design can be compiled and simulated. All tests should be located under [$CL_DIR/verif/tests](./verif/tests) and test targets added to [Makefile.tests](./verif/scripts/Makefile.tests):

```bash
cd $AWS_FPGA_REPO_DIR/hdk/cl/examples/single_register_read_write
export CL_DIR=$(pwd)
cd ${CL_DIR}/verif/scripts
make single_register_read_write_base_test
```

Or specify an [available simulator](./../../../../README.md#supported-tools-versions):

```bash
make single_register_read_write_base_test <SIMULATOR>=1
```
...
All verification work is located under the [verif](./verif) directory. Please see the [Verification](#verification) section for the single_register_read_write verification details.

### STEP 4: Develop Software Tests (OPTIONAL)

single_register_read_write does not yet support standalone software tests. Please refer to other examples such as [CL_DRAM_HBM_DMA](./../cl_dram_hbm_dma/README.md#software)
...
2. Update [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl) with:
   ...
   - Xilinx IP's utilized in the design
     - IP's used in AWS example designs can uncommented be in [synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl) and found under [$AWS_FPGA_REPO_DIR/hdk/common/ip/cl_ip](https://github.com/aws/aws-fpga-resources/tree/Vivado_2024.2-hdk/common/ip/cl_ip)
     - If a design adds new IP's, make sure to add the new `xci` files to the build script [$CL_DIR/build/scripts/synth_single_register_read_write.tcl](./build/scripts/synth_single_register_read_write.tcl)
...
./aws_build_dcp_from_cl.py -c single_register_read_write
```
