...
#ifdef SV_TEST
#include "fpga_pci_sv.h"     /* cl_poke, cl_peek, sv_pause, log_printf … */
#else                        /* <-- compiled when you run on *real* FPGA later */
#include <fpga_mgmt.h>
#include <fpga_pci.h>
#endif
...
void test_main(uint32_t *exit_code)   /* signature must match the SV import   */
{
    bool ok = true;
    uint32_t v, before, after;

    log_printf("=== HW/SW co-simulation test ===\n");

    /* … your five tests exactly as you wrote them … */

    *exit_code = ok ? 0 : 1;          /* tell SV whether we passed */
}
